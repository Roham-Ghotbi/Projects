...
(abs (- (* 98 2) (/ 3 45) (- 1 1000) 1650))             
; expect 455.0666666666666
...
(define (square x) (* x x)) ; redefined it here so that I can use it 
...
(define (has-cycle s)
  ...
  (define (checker lst)
    (cond
     ((null? lst) #f)
     ((eq? lst seen) #t)
     (else (checker (cdr-stream lst)))
     )
  )
  (checker (cdr-stream s))
)

(define s (cons-stream 1 (cons-stream 2 nil)))
(define cycle (cons-stream 1 (cons-stream 2 cycle)))
(has-cycle s)
