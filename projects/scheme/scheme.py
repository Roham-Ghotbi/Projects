...
def scheme_eval(expr, env, _=False): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(scheme_eval(expr.first, env), expr.second.map(eval_short), env)
        # END PROBLEM 5
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
                raise SchemeError('formal parameters and values quantity mismatch!')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target.first] = LambdaProcedure(formals, expressions.second, env)
...
def make_let_frame(bindings, env):
    ...
    check_formals(formals)                  #can go without it aswell
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN Extra Credit
            def eval_short(expr):                       #Just a helper function to use with map
                return scheme_eval(expr, env)

            result = scheme_apply(scheme_eval(expr.first, env), expr.second.map(eval_short), env)
            # END Extra Credit
    return result
